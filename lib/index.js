...
exports.socket = function(type, options) {
  ...
  // fallback to js polling on windows
  if (process.platform === 'win32') polling(sock, 1);
...
function polling(sock, repeat) {
  var timer = setInterval(function () {
    if (sock._zmq.state === zmq.STATE_CLOSED) {
      clearInterval(timer);
    }
    else {
      try {
        ...
        //console.log('zeromq.node polling error');
      }
    }
  }, repeat);
}
